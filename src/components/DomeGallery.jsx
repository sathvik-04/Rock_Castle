...
export default function DomeGallery({
  ...
  // Auto-rotate: slow continuous right-to-left rotation
  useEffect(() => {
    if (!autoRotate) return;

    lastAutoRotateTime.current = null;

    const tick = (timestamp) => {
      // Pause rotation while dragging or viewing enlarged image
      ...
      if (lastAutoRotateTime.current === null) {
        lastAutoRotateTime.current = timestamp;
        autoRotateRAF.current = requestAnimationFrame(tick);
        return;
      }

      const delta = (timestamp - lastAutoRotateTime.current) / 1000; // seconds
      lastAutoRotateTime.current = timestamp;

      // Rotate Y by autoRotateSpeed degrees per second (positive = right-to-left)
